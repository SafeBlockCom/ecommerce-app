...
import { CONSTANTS, HELPER, IMAGE_SRC } from "../../../utils";
...
const findCommonByKey = (arr1, arr2, key) => {
  var uniqueResultArrayObjOne = arr1.filter(function (objOne) {
    return arr2.some(function (objTwo) {
      return objOne[key] === objTwo[key];
    });
  });
  return uniqueResultArrayObjOne;
};
...
const ProductOptions = ({ product, attribute, selectedOption }) => {
  ...
  const handleChangeOption = (option) => {
    ...
    let _isAny = false;
    let variant_ids = product?.variant_ref.filter(
      (e) => e.options === option.id
    );
      ...
      >
        <div className="img-name-price">
          ...
          <div className="name-price">
            ...
            <div className="price">
              <div className="strikeText">{selectedVariant?.price} </div>
            ...
            <div className="update-qty">
              ...
              <div className="cart-actions">
                {maxQty === 0 ? (
                  ...
                  ""
                  // <CartActions detailView={true} /> hold
                )}
              </div>
            </div>
          ...
          <p>
            {parse(
              HELPER.isNotEmpty(selected_variant?.variant_short_description)
                ? selected_variant?.variant_short_description
