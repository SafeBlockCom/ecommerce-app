...
import { Form, Row, Col, Card, CardBody, Input, TabPane } from "reactstrap";
...
const SettingsTab = () => {
  ...
  const onSettingsUpdate = () => {
    ...
    if (
      HELPER.isNotEmpty(logo) &&
      typeof logoCropperRef.current?.cropper !== "undefined"
    ) {
      logoFile = logoCropperRef.current?.cropper.getCroppedCanvas().toDataURL();
      setLogo(logoFile);
    }
    if (
      HELPER.isNotEmpty(banner) &&
      typeof bannerCropperRef.current?.cropper !== "undefined"
    ) {
      bannerFile = bannerCropperRef.current?.cropper
        .getCroppedCanvas()
        .toDataURL();
    ...
    dispatch(
      CLOSET_ACTIONS.CLOSET_UPDATE_SETTINGS(
        {
          name: closetName,
          logo: logoFile,
          banner: bannerFile,
                ...
                <div className="dashboard-detail">
                  <div className="account-setting">
                    ...
                    <Row>
                      <Col sm="12">
                        <button
                          onClick={(e) => onSettingsUpdate(e)}
                          className="btn btn-solid btn-green btn-sm"
                        >
                          Save
                        </button>
                      </Col>
