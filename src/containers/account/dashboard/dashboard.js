...
import { Container, Row, Col, TabContent, Media } from "reactstrap";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    dispatch(CLOSET_ACTIONS.GET_CLOSET_DETAILS(closetRef));
  }, []);

  return HELPER.isEmpty(closet?.name) ? (
    <Loader />
  ) : (
    <CommonLayout
      parent="home"
