...
function updateVariantOptions(
  ...
  return variants.map((variant, key) => {
    return {
      ...
      discounted_price: HELPER.isNotEmpty(discountedPrice[key])
        ? discountedPrice[key]
        : variant?.discounted_price,
      qty: HELPER.isNotEmpty(qty[key]) ? qty[key] : variant?.qty,
      description: HELPER.isNotEmpty(description[key])
        ? description[key]
        : variant?.description,
...
function objectToArr(oldObj, keyName) {
  return Object.entries(oldObj).map(([name, obj]) => ({
    name: keyName,
    ...obj,
  }));
}
...
const VariantsInfo = forwardRef((props, ref) => {
  ...
  useEffect(() => {
    var arr = [
      objectToArr(color, "color"),
      objectToArr({ condition }, "condition"),
      objectToArr(size, "size"),
      objectToArr({ standard }, "standard"),
    ];
    var result = getAllCombinations(arr, 0);
    let _variants = formatVariantOptions(
      result,
      quantity,
      description,
      discountedPrice,
  ...
  useImperativeHandle(ref, () => ({
    handleNextAction() {
      const variantsUpdated = updateVariantOptions(
        variants,
        variantQty,
        variantPrice,
      ...
      dispatch(
        PRODUCT_ACTIONS.ADD_NEW_PRODUCT_DATA(
          activeStep,
          CONSTANTS.PRODUCT_ADDED.VARIANTS,
          {
                          ...
                          <div className="account-setting">
                            ...
                            {variant?.variation.map((variationDetail, key) => {
                              return (
                                <>
                                  <b>
                                    {HELPER.firstLetterCapitalize(
                                      variationDetail?.name
                                    )}
                                  </b>
                                  :{" "}
                                  <span className="variantOption">
                                    {variationDetail?.label} &nbsp;
                                  </span>
                                </>
                              );
                            })}
                        ...
                        <Col lg="2" md="2" sm="2" xs="2">
                          <div className="account-setting">
                            ...
                            <Input
                              name="Outlined"
                              value={
                                variantDiscountedPrice[key] ?? discountedPrice
                              }
                              variant="outlined"
                              type="number"
                              ...
                              onChange={(event) => {
                                ...
                                if (
                                  newValue >= 0 &&
                                  newValue <= 10000 &&
                                  newValue < variantDiscountedPrice[key]
                                )
