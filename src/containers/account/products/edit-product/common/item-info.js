...
const ItemInfo = forwardRef((props, ref) => {
  ...
  const { color, brands, categories, condition, size, standard, addedProduct } =
    useSelector((state) => state.products);

  let value = "";

  const [category, setCategory] = useState(
    addedProduct?.item_information?.category
  );
  ...
  const [subCategory, setSubCategory] = useState(
    addedProduct?.item_information?.subCategory
  );
  const [brand, setBrand] = useState(addedProduct?.item_information?.brand);
  const [productcondition, setCondition] = useState(
    addedProduct?.item_information?.condition
  );
  const [sizeChart, setSizeChart] = useState(
    addedProduct?.item_information?.size
  );
  const [quantity, setQuantity] = useState(
    addedProduct?.item_information?.quantity
  );
  const [standardSize, setStandardSize] = useState(
    addedProduct?.item_information?.standard
  );
  const [productColor, setColor] = useState(
    addedProduct?.item_information?.color
  );
    ...
    handleNextAction() {
      dispatch(
        PRODUCT_ACTIONS.ADD_NEW_PRODUCT_DATA(
          activeStep,
          CONSTANTS.PRODUCT_ADDED.ITEM_INFORMATION,
          {
            category,
            subCategory,
            brand,
          ...
          if (HELPER.isNotEmpty(option.children)) {
            option.children.map((subOption) => {
              formatSubCategories.push({
                parentLabel: HELPER.isNotEmpty(option?.label)
                  ? option?.label
                  : category?.label,
