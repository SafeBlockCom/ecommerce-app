...
const ShipmentLocation = forwardRef((props, ref) => {
  ...
  const { metaCountryList, metaLoading } = useSelector(
    (state) => state.metadata
  );
  const { addedProduct } = useSelector((state) => state.products);

  const [country, setCountry] = useState(
    addedProduct?.shipment_and_location?.country
  );
  const [freeShipping, setFreeShipping] = useState(
    addedProduct?.shipment_and_location?.freeShipping
  );
  const [worldWideShipping, setWorldWideShipping] = React.useState(
    addedProduct?.shipment_and_location?.worldWideShipping
  );
  const [shippingPrice, setShippingPrice] = useState(
    addedProduct?.shipment_and_location?.shippingPrice
  );
    ...
    handleNextAction() {
      dispatch(
        PRODUCT_ACTIONS.ADD_NEW_PRODUCT_DATA(
          activeStep,
          CONSTANTS.PRODUCT_ADDED.SHIPMENT_AND_LOCATION,
          {
            country,
            freeShipping,
            shippingPrice,
                ...
                <div className="">
                  ...
                  <Row className="mt-4 ml-1 mr-1">
                    <Col
                      ...
                      sm="12"
                      xs="12"
                      className="shipment-section pt-4"
                    >
