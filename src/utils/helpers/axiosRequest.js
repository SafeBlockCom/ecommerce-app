...
const makeRequest =
  (instance) =>
  (method, url, token, allow_error_pages, ...params) => {
    const locale = LOCAL_STORAGE_SERVICE._getFromLocalStorage("locale");
    // Set Language Headers
    mainInstance.defaults.headers.common["Accept-Language"] = HELPER.isNotEmpty(
      locale
    )
      ? locale
      : "en";
    // Set Access token
    const access_token = COOKIE_STORAGE_SERVICE._getAccessTokenFromSession();
    if (access_token) {
...
const API_REQUEST =
  (method, url, token, allow_error_pages) =>
  (...params) => {
