...
function SELECT_PRODUCT_VARIANT(product, variant) {
  return (dispatch) => {
    dispatch(request(product, variant));
  };
  function request(product, variant) {
    // return { type: CART_CONSTANTS.SELECT_PRODUCT_VARIANT, product, variant };
  }
}
...
function ADD_NEW_PRODUCT_DATA(step, type, data) {
  ...
  function request(action, response, step) {
    return {
      type: PRODUCTS_CONSTANTS.PRODUCT_DATA_ADDED,
      action,
      response,
      activeStep: step,
    };
