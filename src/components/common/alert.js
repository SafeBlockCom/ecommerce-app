...
function AlertComponent() {
  ...
  return HELPER.isNotEmpty(message) && group !== CONSTANTS.ERROR_TYPE.TOAST ? (
    <div className="row">
      <div className={`col-12 alert_classes `}>
        <div
          className={`alertCustom  ${HELPER.isEmpty(type) ? "default" : type}`}
          role="alert"
        >
          <div
            className={`alertAction ${HELPER.isEmpty(type) ? "default" : type}`}
          >
            <span
              className={`${
                type === "error"
