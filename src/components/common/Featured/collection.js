...
export default function FeaturedByCollection(props) {
  ...
  const [activeCollectionProductList, setActiveCollectionProductList] =
    useState([]);

  useEffect(() => {
    if (HELPER.isNotEmpty(featuredProducts)) {
      setActiveCollectionProductList(
        featuredProducts[activeCollectionIndex].data
      );
    }
  }, [featuredProducts]);
  ...
  return (
    <Row className="multiple-slider" key={"key"}>
            ...
            return (
              <h5
                className={`title-border ${
                  activeCollectionIndex === i ? "active" : ""
                }`}
      ...
      <Col
        ...
        md="12"
        sm="12"
        className={"featured_sq ratio_square featured-section"}
      >
                ...
                {activeCollectionProductList.map((data, key) => {
                  return (
                    <Col
                      ...
                      xs="12"
                      className="mb-3"
                      key={"featured-col-" + key}
                    >
                      <div>
                        <ProductItem
                          product={data}
                          addToCompare={() => contextCompare.addToCompare(data)}
                          addWishlist={() => {
