  ...
  return (
    <>
      ...
      <section className="main-banner d-flex noPadding noMargin">
        <MobileCollectionBanner
          first
          bannerItem={caraouselBanner.slice(0, 2)}
          keyId={1}
        />
        <MobileCollectionBanner
          bannerItem={caraouselBanner.slice(2, 4)}
          keyId={2}
        />
      </section>
    </>
        ...
        <Row className="partition3">
          {first
            ? bannerItem.map((data, i) => {
                return <Banner key={i} data={data} keyId={i + keyId} />;
              })
            : bannerItem.map((data, i) => {
                return <Banner key={i} data={data} keyId={i + keyId + 1} />;
              })}
        </Row>
      </Container>
    </section>
